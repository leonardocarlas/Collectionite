...
if( isset($_POST['collezione']) ) {
    ...
    */
    ...
    foreach ( $lista as $id_wanted_list => $array_informazioni_carte ) {
        ...
        if ( $array_informazioni_carte[5] == "//static.cardmarket.com/img/57dd2eebb3f9e891f2febb85e9d2a358/items/1041/PR/317006.jpg") {
            echo '';
        }
        else {
...
function search_card_in_wl($id_collezione) {
    ...
    if( isset($_POST['testo_wantedlist']) &&  isset($_POST['idcollezione']) ) {
        ...
        if (empty($testo_cercato))
        {
            ...
            // Cercare l'id della carta più stupido dell'intero db 317006, una carta di My Little Pony
            // Selezionare il max Idwantedlist presente
            // INSERT INTO wanted_list (Description, Idcollection) VALUES ("Prova Prova zio", 1)
            // INSERT INTO desires (Iduser, Idcard, Idwantedlist, Min_condition, Language_wanted, Max_price) VALUES(317006, 1, 7, 1,1,0)
            ...
            if(!mysqli_stmt_prepare($stmt, $sql)){
                ...
                // Selezione dell'id della wanted_list

                $sql = " SELECT Idwantedlist 
                            FROM wanted_list
                            ORDER BY Idwantedlist DESC
                            LIMIT 1";
                ...
                if(!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    if($result->num_rows > 0){
                        ...
                        $id_latest_wanted_list = $row['Idwantedlist'] ;

                        $id_user = $_SESSION['idusersession'];

                        // Inserimento carta fittizia
        
                        $sql = " INSERT INTO desires (Iduser, Idcard, Idwantedlist, Min_condition, Language_wanted, Max_price) 
                        VALUES( ? ,317006, ? ,1,1,0) ";
                        $stmt = mysqli_stmt_init($connessione);
                        ...
                        if(!mysqli_stmt_prepare($stmt, $sql)){
                            ...
                            mysqli_stmt_bind_param($stmt, "ii", $id_user, $id_latest_wanted_list);
                            mysqli_stmt_execute($stmt);
